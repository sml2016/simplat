   ...
   <div class="fondo1">
    <form type="POST" role="form" method="POST" action="{{ url('/registro') }}">
      ...
      @if ($daycareCount >= $settings->daycare_limit)
        <h3>Servicio de guarderia ya esta lleno.</h3>
      @else
        <div class="section2R">
        ...
        @else
          <div class="section3R" style="display:none">
            <h1>Solo queda un lugar en la guarderia</h1>  
            <h2>El menor debe tener la edad entre 3 y 11 años</h2>
            ...
            <div class="col-xs-8 col-xs-offset-2 col-sm-8 col-sm-offset-2 col-md-6 col-md-offset-3">
              ...
              <div class="nino1">
                ...
                <select name="child1sex" class="form-control input-lg">
                  <option value="0" {{ Request::old('child1sex') == '0' ? 'selected' : '' }}>Masculino</option>
                  <option value="1" {{ Request::old('child1sex') == '1' ? 'selected' : '' }}>Femenino</option>
                </select>
              </div>
            </div>
          </div>
